...
public class ReservasService {
    ...
    public List<Reservas> getReservasByEstado(String estado) {
    ...
    public List<Reservas> getReservasByFecha(Date fechareserva) {
        List<Reservas> reservas = reservasRepository.findByFechareserva(fechareserva);
        if (reservas.isEmpty()) {
            // error
        }
        return reservas;
    ...
    public void cancelarReserva(Long id) {
        int patchedRows = reservasRepository.patchEspacios(null, null, null, "Cancelado", id);
        if (patchedRows == 0) {
            // error
        }
    }
